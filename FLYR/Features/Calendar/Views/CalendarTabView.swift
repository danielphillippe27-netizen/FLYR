...
private struct MonthDayCell: View {
        ...
        if isInDisplayedMonth && shouldShowSessionStatus {
            if hasSession {
                Image(systemName: "checkmark.circle.fill")
                    .font(.system(size: mode == .compact ? 12 : 10, weight: .bold))
                    .foregroundColor(Color(red: 0.2, green: 0.78, blue: 0.35))
                    .frame(width: mode == .compact ? 14 : 12, height: mode == .compact ? 14 : 12)
                    .background(Circle().fill(Color(uiColor: .systemBackground)))
                    .accessibilityHidden(true)
            } else {
                Text("x")
                    .font(.system(size: mode == .compact ? 9 : 8, weight: .bold))
    ...
    private var monthMarkerRow: some View {
        HStack(spacing: 3) {
            monthActivityGlyph
            ForEach(monthVisibleItems.prefix(2)) { item in
                monthMarker(for: item)
            }
        }
    ...
    @ViewBuilder
    private func monthMarker(for item: CalendarItem) -> some View {
        if let letter = monthLetter(for: item) {
            Text(letter)
                .font(.system(size: mode == .compact ? 9 : 8, weight: .bold))
    ...
    private var dotColors: [Color] {
        if monthVisibleItems.count > 3 {
            return Array(monthVisibleItems.prefix(2).map { CalendarColorKey.color(for: $0.colorKey) }) + [Color(uiColor: .systemGray)]
        }
        return monthVisibleItems.prefix(3).map { CalendarColorKey.color(for: $0.colorKey) }
    }

    private var monthVisibleItems: [CalendarItem] {
        items.filter { $0.kind != .session }
    }
