...
struct BuildingCircleToggle: View {
    ...
    private var selectedOptionForeground: Color { isLightMode ? .black : .white }
...
struct CampaignMapView: View {
    ...
    private var visibleAddressFeatures: [AddressFeature] {
        featuresService.addresses(for: campaignId)?.features ?? []
    }
    ...
    private var visibleParcelFeatures: [ParcelFeature] {
        parcelsAnnotatingVisibleAddressLinks(from: featuresService.parcels(for: campaignId)?.features ?? [])
    }
    ...
    private var parcelsRenderSignature: String {
        let features = visibleParcelFeatures
        guard !features.isEmpty else { return "none" }
        let ids = features.compactMap { feature in
        ...
        displayMode == .buildings
            ...
            && (
                !visibleBuildingFeatures.isEmpty ||
                    (!usesStandardPinsRenderer && currentDiamondManifest?.hasRenderablePMTilesGeometry == true)
            )
    }
            ...
            if sessionManager.sessionId != nil {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    if sessionManager.isDemoSession {
                        ...
                        .padding(.horizontal, 12)
                        .padding(.vertical, 8)
                        .background(isLightMode ? Color.black.opacity(0.86) : Color.darkSurface.opacity(0.96))
    ...
    private func scheduleLayerVisibilityReassert(attempt: Int = 0) {
        ...
        guard !hasBuildingsLayer || !hasAddressesLayer || !hasSelectedAddressesLayer || !hasParcelsFillLayer || !hasParcelsLineLayer else {
    ...
    private func updateLayerVisibility(for mode: DisplayMode) {
        ...
        let hasParcelsFillLayer = map.allLayerIdentifiers.contains(where: { $0.id == MapLayerManager.parcelsFillLayerId })
        let hasParcelsLineLayer = map.allLayerIdentifiers.contains(where: { $0.id == MapLayerManager.parcelsLineLayerId })
        let hasDiamondBuildings = !usesStandardPinsRenderer && currentDiamondManifest?.hasRenderablePMTilesGeometry == true
        let hasDiamondAddresses = !usesStandardPinsRenderer && currentDiamondManifest?.hasRenderablePMTilesAddresses == true
    ...
    private var loadingOverlay: some View {
        let shouldShowLoader = !quickStartEnabled && (
            isInitialMapPreparing ||
                (featuresService.isLoading && !hasFirstCampaignDrawData)
        )
        if shouldShowLoader {
            ZStack {
                (shouldCoverInitialCampaignMap ? (isLightMode ? Color.black.opacity(0.96) : Color.darkSurface.opacity(0.98)) : Color.clear)
                    .ignoresSafeArea()
                ...
                .background(
                    RoundedRectangle(cornerRadius: 8)
                        .fill(isLightMode ? Color.black.opacity(0.68) : Color.darkSurfaceElevated.opacity(0.96))
                )
                .offset(y: -56)
            }
    ...
    private func waitForProvisionReadinessBeforeInitialBundleIfNeeded(
        ...
        do {
            ...
            await applyProvisionGateState(state, campaignUUID: campaignUUID)
            let usable = state.provisionStatus == .ready && (state.provisionPhase?.isMapUsable ?? true)
            trace.end(status: usable ? "ready" : "returned_not_ready", fields: [
    ...
    private func performMapDataUpdate() {
        ...
        let trace = PerfTrace.begin("campaign_open", "update_map_data", fields: [
            "campaign": campaignId,
            "displayMode": displayMode.rawValue,
            "visibleBuildings": visibleBuildingFeatures.count,
            "visibleAddresses": visibleAddressFeatures.count
        ])
           ...
           let addressesData = addressDataForLayerCache(features: addressFeaturesForDisplay) {
            manager.updateAddresses(
                ...
                orderedAddressIdsByBuilding: buildingAddressMap
                  ...
                  let buildingsData = visibleBuildingsGeoJSONData() {
        ...
        if let roads = featuresService.roads(for: campaignId), !roads.features.isEmpty,
        ...
        trace.end(status: "complete", fields: [
            "visibleBuildings": visibleBuildingFeatures.count,
            "visibleAddresses": visibleAddressFeatures.count,
            "visibleParcels": visibleParcelFeatures.count
        ])
    }
    ...
    private func refreshVisibleBuildingRenderMonitoring(reset: Bool) {
        ...
        let hasDiamondGeometry = !usesStandardPinsRenderer && currentDiamondManifest?.hasRenderablePMTilesGeometry == true
    ...
    private func parcelsAnnotatingVisibleAddressLinks(from parcels: [ParcelFeature]) -> [ParcelFeature] {
        ...
        var addressIdsByParcelIndex: [Int: [String]] = [:]

        for (index, parcel) in parcels.enumerated() {
            let parcelAddressIds = ([parcel.properties.addressId] + (parcel.properties.addressIds ?? []))
                .compactMap(normalizedMapFeatureIdentifier)
                .filter { addressIds.contains($0) }
            let uniqueParcelAddressIds = Array(Set(parcelAddressIds)).sorted()
            if let addressId = uniqueParcelAddressIds.first {
                addressIdByParcelIndex[index] = addressId
                addressIdsByParcelIndex[index] = uniqueParcelAddressIds
            }
        }
        ...
        for address in addressCoordinates where !addressIdByParcelIndex.values.contains(address.id) {
            ...
            addressIdsByParcelIndex[index] = Array(
                Set((addressIdsByParcelIndex[index] ?? []) + [address.id])
            ).sorted()
        }

        return parcels.enumerated().map { index, parcel in
            guard let addressId = addressIdByParcelIndex[index] else { return parcel }
            return parcel.withAddressId(addressId, addressIds: addressIdsByParcelIndex[index])
        }
    }
    ...
    private func handleMapLongPressBegan(at point: CGPoint) {
        ...
        startAddHouseFlow(with: context)
        openReverseGeocodedManualAddressDraft(
            at: seed,
            linkedBuildingId: nil,
            fallbackAddressText: nil,
            onlyIfPlacementStillMatches: true
        )
    }
    ...
    private func presentNearbyPickerForUnlinkedBuilding(
        ...
        let building = feature.properties
        guard !buildingHasAttachedAddress(building) else { return false }
    ...
    private func presentParcelLinkedAddressSelection(
        ...
        highlightAddressCluster(
            addressIds: linkedAddressIds,
            preferredAddressId: enrichedAddress.addressId,
            haptic: false
        )
    }
    ...
    private func highlightAddressCluster(
        ...
        let exactFeature = buildingFeature(matchingAddressId: preferredAddressId)
        applyLinkedHighlight(
    ...
    private func handleManualAddressSaved(
        ...
        if let linkedBuildingId = response.linkedBuildingId {
            ...
            presentSavedStandaloneManualAddress(response.address, coordinate: coordinate)
        }
    }
...
final class CampaignMapboxContainerView: UIView {
    ...
    func installMapView(mapInitOptions: MapInitOptions) -> MapView {
        ...
        let mapView = MapView(frame: CGRect(origin: .zero, size: size), mapInitOptions: mapInitOptions)
...
struct LocationCardView: View {
    ...
    private var hasEditableAddressForDelete: Bool {
        editableAddress != nil || addressId != nil || !dataService.buildingData.addresses.isEmpty
    }

    private var addressEditDeleteAction: LocationCardToolsAction {
        guard canDeleteBuilding else { return .deleteAddress }
        return hasEditableAddressForDelete ? .deleteUnit : .deleteBuilding
    }
    ...
    private var cardViewWithDataLoading: some View {
        baseCardView
            ...
            .task(id: dataRequestKey) {
                ...
                if NetworkMonitor.shared.isOnline && entitlementsService.canUsePro {
                ...
                trace.end(status: "complete", fields: [
                    ...
                    "detailsLoaded": buildingDetails != nil,
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 8) {
                    ...
                    addressEditActionButton("Delete", icon: "trash", tint: .red) {
                        addressEditAction(addressEditDeleteAction)
                    }
                }
            }
            .padding(14)
