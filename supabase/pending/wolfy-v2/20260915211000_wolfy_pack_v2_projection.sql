...
BEGIN;
CREATE FUNCTION public.wolfy_pack_snapshot(p_campaign uuid,p_after timestamptz DEFAULT NULL) RETURNS jsonb
...
BEGIN
 ...
 WITH roster AS (
  ...
  SELECT r.user_id,coalesce(nullif(p.first_name,''),nullif(split_part(p.full_name,' ',1),''),'Rep') AS first_name,
   ...
   (SELECT max(occurred_at) FROM wolfy_v2_ledger WHERE user_id=r.user_id AND campaign_id=p_campaign) AS last_activity_at
 ...
 SELECT jsonb_build_object('version',2,'enabled',true,'campaign_id',p_campaign,'server_time',now(),
  ...
  'events',coalesce((SELECT jsonb_agg(to_jsonb(e)) FROM (
    SELECT id,campaign_id,actor_id,recipient_id,kind,created_at,starts_at,expires_at,animation_seed
    FROM wolfy_pack_events WHERE campaign_id=p_campaign
      ...
      AND expires_at>now() ORDER BY created_at,id LIMIT 50
   ) e),'[]'::jsonb)) INTO result;
 RETURN result;
END $$;
