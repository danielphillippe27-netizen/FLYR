-- Fix leaderboard "Doors" metric to read door hits from sessions.doors_hit
-- while preserving the legacy flyers metric for existing web consumers.
...
BEGIN
    ...
    IF p_workspace_id IS NOT NULL THEN
        ...
        session_stats AS (
            SELECT
                s.user_id,
                COALESCE(SUM(GREATEST(COALESCE(s.doors_hit, s.completed_count, s.flyers_delivered, 0), 0)), 0)::INTEGER AS session_doorknocks,
                COALESCE(SUM(GREATEST(COALESCE(s.flyers_delivered, 0), 0)), 0)::INTEGER AS session_flyers,
                COALESCE(SUM(GREATEST(COALESCE(s.conversations, 0), 0)), 0)::INTEGER AS session_conversations,
            ...
            WHERE s.start_time >= v_start_date
              AND (
                  ...
                  OR (
                      ...
                      AND s.user_id IN (SELECT user_id FROM workspace_user_ids)
                  )
              )
            GROUP BY s.user_id
        ),
        ranked_users AS (
            SELECT
                ...
                COALESCE(ss.session_doorknocks, 0) AS user_doorknocks,
                ...
                jsonb_build_object(
                    'doorknocks', COALESCE(ss.session_doorknocks, 0),
            ...
            WHERE
                COALESCE(ss.session_doorknocks, 0) > 0
                OR COALESCE(ss.session_flyers, 0) > 0
                OR COALESCE(ss.session_conversations, 0) > 0
        )
        SELECT
            ru.user_id,
            ru.display_name,
            ...
            (ROW_NUMBER() OVER (
                ORDER BY
                    CASE p_metric
                        WHEN 'doorknocks' THEN ru.user_doorknocks
...
COMMENT ON FUNCTION public.get_leaderboard(TEXT, TEXT, UUID) IS 'Leaderboard with optional workspace scope; adds doorknocks metric from sessions.doors_hit while preserving flyers.';
