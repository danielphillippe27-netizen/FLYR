...
BEGIN
    ...
    streak_groups AS (
        SELECT
            MIN(d.session_day) AS streak_start,
            MAX(d.session_day) AS streak_end,
            COUNT(*)::INTEGER AS streak_length
        FROM (
            SELECT
                dd.session_day,
                dd.session_day - (ROW_NUMBER() OVER (ORDER BY dd.session_day)::INTEGER * INTERVAL '1 day') AS streak_group
            FROM distinct_days dd
        ) d
        GROUP BY d.streak_group
    ),
    streak_summary AS (
        SELECT
            ...
            COALESCE(
                MAX(
                    CASE
                        WHEN sg.streak_end >= CURRENT_DATE - 1 THEN sg.streak_length
                        ELSE 0
                    END
                ),
                0
