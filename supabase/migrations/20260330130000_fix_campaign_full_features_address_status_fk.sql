-- Fix rpc_get_campaign_full_features after address_statuses.address_id was
-- renamed to campaign_address_id. Without this, the buildings API falls back
-- to empty for Gold campaigns even when linking succeeded.
...
BEGIN
    WITH latest_address_status AS (
        SELECT DISTINCT ON (s.campaign_address_id)
            s.campaign_address_id,
            s.status
        FROM public.address_statuses s
        WHERE s.campaign_id = p_campaign_id
        ORDER BY s.campaign_address_id, s.updated_at DESC, s.created_at DESC
    ),
    gold_features AS (
...
'Returns a GeoJSON FeatureCollection for a campaign. Includes gold buildings, campaign buildings, and free address points with status derived from address_statuses.campaign_address_id.';
