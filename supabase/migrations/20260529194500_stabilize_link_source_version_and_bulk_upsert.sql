-- Keep canonical link source_version tied to linker inputs only, not mutable
-- campaign output fields such as provision_phase, map_mode, or title updates.
...
BEGIN
  v_received := COALESCE(jsonb_array_length(
    CASE
  ...
  RETURN jsonb_build_object('received', v_received, 'upserted', v_inserted);
END;
$$;
