...
-- The previous canonical wrapper delegated with SELECT * to get_leaderboard_base.
-- That is brittle across deployed databases whose 3-argument leaderboard function
-- may have a different projection. Keep one public PostgREST-facing signature and
-- define the full projection here.
...
COMMENT ON FUNCTION public.get_leaderboard(TEXT, TEXT, UUID, INTEGER, INTEGER) IS
    'Canonical leaderboard RPC signature for PostgREST; self-contained projection for iOS leaderboard.';
