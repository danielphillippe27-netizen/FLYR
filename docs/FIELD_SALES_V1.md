...
The additive migration is mirrored byte-for-byte in both repositories as `20260915190000_field_sales_v1.sql`. Apply it **once per actual Supabase database**, not twice merely because both clients contain it. The default is disabled and no workspace is enabled by the migration. Clients cannot enable the gate.
...
No production migration, deployment, App Store submission, or production gate activation was performed by this implementation run. A separate concurrently-created `20260915200000_wolfy_field_sales_rewards.sql` migration is outside this V1 scope and must not be included in this release. The working trees contain unrelated changes; use a scoped release rather than deploying them wholesale. The public iOS simulator build passed. Whole-web TypeScript checking also reports existing errors outside the Sales changes; resolve those as part of a complete web release check.
