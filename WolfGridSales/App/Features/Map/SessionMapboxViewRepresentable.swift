...
struct SessionMapboxViewRepresentable: UIViewRepresentable {
    ...
    var cameraDragEvent: MapPitchBearingDragEvent?
    ...
    func updateUIView(_ mapView: MapView, context: Context) {
        ...
        context.coordinator.applyCameraDragEvent(cameraDragEvent)
    }
    ...
    class Coordinator {
        ...
        private var lastCameraDragEventId: UUID?
        private var manualCameraPose: CampaignMapCameraPose?
        ...
        func updateCamera(location: CLLocation?, headingState: MapHeadingPresentationState) {
            ...
            let heading = manualCameraPose?.bearing ?? headingState.heading ?? lastCameraSnapshot?.heading ?? 0
            let pitch = manualCameraPose?.pitch ?? 60.0
            ...
            if let map = mapView.mapboxMap {
                MapTheme.applyLightModeShadowPolicy(to: map, pitch: pitch)
            }
        }

        func applyCameraDragEvent(_ event: MapPitchBearingDragEvent?) {
            guard let event,
                  event.id != lastCameraDragEventId,
                  let mapView,
                  let map = mapView.mapboxMap else {
                return
            }
            lastCameraDragEventId = event.id
            ...
            manualCameraPose = pose
            map.setCamera(to: CameraOptions(bearing: pose.bearing, pitch: pose.pitch))
            MapTheme.applyLightModeShadowPolicy(to: map, pitch: pose.pitch)
        }
