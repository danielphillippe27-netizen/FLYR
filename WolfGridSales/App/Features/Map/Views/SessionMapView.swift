...
struct SessionMapView: View {
    ...
    @State private var cameraDragEvent: MapPitchBearingDragEvent?
    ...
    var body: some View {
        ZStack {
            SessionMapboxViewRepresentable(
                ...
                cameraDragEvent: cameraDragEvent
            )
            ...
            VStack(spacing: 0) {
                HStack {
                    Spacer(minLength: 0)
                    FlyrMapPitchBearingControl { dragAmount in
                        cameraDragEvent = MapPitchBearingDragEvent(dragAmount: dragAmount)
                    }
                    .padding(.top, 56)
                    Button {
                        HapticManager.light()
