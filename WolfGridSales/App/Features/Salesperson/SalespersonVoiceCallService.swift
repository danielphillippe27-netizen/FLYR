...
extension SalespersonVoiceCallService: PKPushRegistryDelegate {
    ...
    func pushRegistry(
        ...
        if TelnyxPushPayload.isMissedCall(payload.dictionaryPayload) {
            handleMissedCallPush(payload.dictionaryPayload)
            return
        }
    ...
    /// Telnyx 4.x can send a second VoIP push after the caller hangs up before
    /// the app answers. Apple requires every VoIP push to be reported to
    /// CallKit, so report a short-lived placeholder and dismiss the stale call.
    private func handleMissedCallPush(_ payload: [AnyHashable: Any]) {
        let originalCallID = TelnyxPushPayload.callID(from: payload)
        let placeholderCallID = UUID()
        let update = CXCallUpdate()
        update.remoteHandle = CXHandle(type: .generic, value: " ")
        update.localizedCallerName = "Missed call"
        ...
        provider.reportNewIncomingCall(with: placeholderCallID, update: update) { _ in
            Task { @MainActor in
                if let originalCallID {
                    self.provider.reportCall(
                        with: originalCallID,
                        endedAt: Date(),
                        reason: .answeredElsewhere
                    )
                    self.reportMissedCallIfNeeded(
                        callId: originalCallID,
                        endedAt: Date(),
                        reason: "missed"
                    )
                    self.clearCall(uuid: originalCallID)
                }
                self.provider.reportCall(
                    with: placeholderCallID,
                    endedAt: Date(),
                    reason: .answeredElsewhere
                )
                self.completePendingPushIfNeeded()
    ...
    private func reportIncomingPushPlaceholder(metadata: [String: Any]) {
        ...
        let caller = callerName?.isEmpty == false ? callerName! : callerNumber?.isEmpty == false ? callerNumber! : "Incoming WolfGrid call"
...
private enum VoiceCallError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .microphoneDenied:
            return "Microphone access is required for Telnyx voice calls. Enable microphone permission for WolfGrid in iOS Settings."
