...
actor CRMIntegrationManager {
    ...
    func fetchProviderCatalog() async throws -> [IntegrationCatalogProvider] {
        let data = try await invokeBackendRoute(
            path: "/api/integrations/providers",
            method: "GET"
        ) { _, data in
            String(data: data, encoding: .utf8) ?? "Unable to load CRM providers."
        }
        return try JSONDecoder().decode([IntegrationCatalogProvider].self, from: data)
    }
    ...
    func connectCatalogProvider(_ provider: IntegrationProvider, credential: String) async throws {
        let key: String
        switch provider {
        case .zapier: key = "webhookUrl"
        case .boldtrail, .companycam: key = "apiToken"
        default: key = "apiKey"
        }
        _ = try await invokeBackendRoute(
            path: "/api/integrations/\(provider.rawValue)/connect",
            method: "POST",
            body: [key: credential]
        ) { _, data in
            String(data: data, encoding: .utf8) ?? "Unable to connect \(provider.displayName)."
        }
    }
    ...
    func sendMondayTestLead(userId: UUID) async throws -> String {
        ...
        let data = try await invokeEdgeFunction(name: "crm_sync", body: [
            "lead": [
                "id": UUID().uuidString,
                "name": "WolfGrid Monday Test Lead",
                "phone": "5555555555",
                "email": "test+\(timestamp)@wolfgrid.app",
                "address": "123 Test St, Test City",
                "source": "WolfGrid iOS Monday Test",
                "notes": "Test lead from WolfGrid iOS Monday.com sync settings.",
    ...
    func disconnect(userId: UUID, provider: IntegrationProvider) async throws {
        if [.jobnimbus, .companycam, .jobber, .acculynx, .rooflink].contains(provider) {
            _ = try await invokeBackendRoute(
                ...
                String(data: data, encoding: .utf8) ?? "Unable to disconnect \(provider.displayName)."
            }
            return
        }
        if provider == .monday {
