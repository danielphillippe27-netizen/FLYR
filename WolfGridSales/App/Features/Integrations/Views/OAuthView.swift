...
struct OAuthURLBuilder {
    ...
    static func authURL(for provider: IntegrationProvider) async throws -> URL {
        switch provider {
        case .hubspot, .companycam, .jobber:
            return try await HubSpotOAuthAPI.shared.fetchAuthorizeURL(provider: provider, platform: "ios")
        case .monday:
...
private final class HubSpotOAuthAPI {
    ...
    func fetchAuthorizeURL(provider: IntegrationProvider = .hubspot, platform: String = "ios") async throws -> URL {
        let session = try await SupabaseManager.shared.client.auth.session
        var components = URLComponents(string: "\(requestBaseURL)/api/integrations/\(provider.rawValue)/oauth/start")!
        ...
        guard let url = components.url else {
            throw OAuthURLBuilder.OAuthBuildError.invalidURL(provider.displayName)
        }
        ...
        guard (200...299).contains(http.statusCode) else {
            throw NSError(
                ...
                userInfo: [NSLocalizedDescriptionKey: payload?.error ?? "Unable to start \(provider.displayName) OAuth."]
            )
        }
        ...
        guard let rawURL = payload?.authorizeURL, let authorizeURL = URL(string: rawURL) else {
            throw NSError(
                domain: "HubSpotOAuthAPI",
                code: http.statusCode,
                userInfo: [NSLocalizedDescriptionKey: payload?.error ?? "\(provider.displayName) OAuth did not return an authorize URL."]
            )
        }
