...
actor ContactsService {
    ...
    /// - Parameters:
    ///   - userID: Required owner scope. Contacts/leads are never shared merely because users share a workspace.
    ///   - workspaceId: Optional secondary filter within the current user's contacts.
    ...
    private static func merged(existing: Contact, incoming: Contact) -> Contact {
        Contact(
            ...
            company: preferredNonEmptyOptional(incoming.company, fallback: existing.company),
            ...
            leadKind: preferredNonEmptyOptional(incoming.leadKind, fallback: existing.leadKind),
            ...
            appointmentAt: incoming.appointmentAt ?? existing.appointmentAt,
    ...
    private static func normalized(_ contact: Contact, overrideAddressId: UUID?) -> Contact {
        Contact(
            ...
            company: normalizedOptional(contact.company),
            ...
            leadKind: normalizedOptional(contact.leadKind),
    ...
    private func performRemoteAddContact(
        ...
        var insertData: [String: AnyCodable] = [
            ...
            "company": AnyCodable(contactToInsert.company as Any),
            ...
            "follow_up_at": AnyCodable(contactToInsert.followUpAt as Any),
            "appointment_at": AnyCodable(contactToInsert.appointmentAt as Any)
        ]
        if let leadKind = contactToInsert.leadKind {
            insertData["lead_kind"] = AnyCodable(leadKind)
        }
    ...
    private func performRemoteUpdateContact(_ contact: Contact, addressId: UUID? = nil) async throws -> Contact? {
        ...
        if let leadKind = contact.leadKind {
            updateData["lead_kind"] = AnyCodable(leadKind)
        }
