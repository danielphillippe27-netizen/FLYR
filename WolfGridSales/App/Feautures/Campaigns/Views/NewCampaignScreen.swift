...
struct NewCampaignScreen: View {
    ...
    @State private var showZeroHomeStandardModePrompt = false
    ...
    var body: some View {
        ...
        .alert("No homes found", isPresented: $showZeroHomeStandardModePrompt) {
            Button("Start Now") {
                Task { await openZeroHomeCampaignInStandardMode() }
            }
            Button("Exit", role: .cancel) {
                dismiss()
            }
        } message: {
            Text("We couldn't find homes in this territory. Use Standard to start now and add homes with pins.")
        }
        .overlay {
    ...
    @MainActor
    private func openZeroHomeCampaignInStandardMode() async {
        guard var campaign = createdCampaign else { return }
        do {
            try await CampaignsAPI.shared.enableStandardPinsMode(campaignId: campaign.id)
            campaign.mapMode = .standardPins
            campaign.standardModeRecommended = true
            campaignMapDataReady = true
            provisionComplete = true
            provisionFailed = false
            createdCampaign = campaign
            ...
            createHook.error = "Standard could not be started: \(error.localizedDescription)"
        }
    }
