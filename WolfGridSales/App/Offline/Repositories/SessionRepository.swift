...
final class SessionRepository {
    ...
    func updateSessionGoalAmount(id: UUID, goalAmount: Int) async {
        let now = Date()
        try? await dbQueue.write { db in
            guard let payloadJSON = try String.fetchOne(
                db,
                sql: "SELECT payload_json FROM local_sessions WHERE id = ?",
                arguments: [id.uuidString]
            ),
                  let payload = OfflineJSONCodec.decode(OfflineSessionPayload.self, from: payloadJSON) else {
                return
            }

            let updatedPayload = OfflineSessionPayload(
                id: payload.id,
                userId: payload.userId,
                campaignId: payload.campaignId,
                targetBuildings: payload.targetBuildings,
                ...
                routeAssignmentId: payload.routeAssignmentId,
                farmExecutionContext: payload.farmExecutionContext,
            ...
            try db.execute(
                ...
                """,
                arguments: [
                    OfflineJSONCodec.encode(updatedPayload),
                    OfflineDateCodec.string(from: now),
                    id.uuidString
                ]
            )
