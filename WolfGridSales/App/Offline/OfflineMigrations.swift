...
enum OfflineMigrations {
    static func migrator() -> DatabaseMigrator {
        ...
        migrator.registerMigration("cached_contacts_company_v1") { db in
            let existingColumns = Set(try db.columns(in: "cached_contacts").map(\.name))
            if !existingColumns.contains("company") {
                try db.alter(table: "cached_contacts") { table in
                    table.add(column: "company", .text)
                }
            }
        }
