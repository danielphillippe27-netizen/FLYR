...
**Telnyx missed calls:** `/api/dialer/calls` records native iOS call events and `/api/webhooks/telnyx/calls` can ingest Telnyx call lifecycle webhooks. Missed inbound calls are stored in `dialer_calls` and surfaced in `/api/inbox` under Calls/All. Configure the Telnyx voice/call webhook URL as `https://www.flyrpro.app/api/webhooks/telnyx/calls`.
