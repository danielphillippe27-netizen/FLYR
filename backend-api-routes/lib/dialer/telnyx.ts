...
class TelnyxRequestError extends Error {
  ...
  constructor(message: string, options?: { isAlphanumericSenderError?: boolean; status?: number }) {
    super(message);
...
export type TelnyxRecordingResult = {
  id: string;
  mp3Url: string | null;
  wavUrl: string | null;
  status: string;
...
function parseTelnyxRecording(data: Record<string, unknown>, fallbackId = ''): TelnyxRecordingResult {
  const downloadUrls = data.download_urls && typeof data.download_urls === 'object'
    ? data.download_urls as Record<string, unknown>
    : {};
  const durationMillis = typeof data.duration_millis === 'number'
    ? data.duration_millis
    : Number(data.duration_millis);
  const channelValue = typeof data.channels === 'number'
    ? data.channels
    : data.channels === 'dual'
      ? 2
  ...
  return {
    ...
    mp3Url: typeof downloadUrls.mp3 === 'string' ? downloadUrls.mp3 : null,
    wavUrl: typeof downloadUrls.wav === 'string' ? downloadUrls.wav : null,
    status: typeof data.status === 'string' ? data.status : 'completed',
    durationSeconds: Number.isFinite(durationMillis) ? Math.round(durationMillis / 1000) : null,
    channels: Number.isFinite(channelValue) ? channelValue : null,
...
export async function startTelnyxCallRecording(
  callControlId: string,
  params?: { commandId?: string; clientState?: string | null }
): Promise<Record<string, unknown>> {
  return telnyxFetch<Record<string, unknown>>(
...
export async function deleteTelnyxCallRecording(recordingId: string): Promise<void> {
  try {
    await telnyxFetch<Record<string, unknown>>(
      `/recordings/${encodeURIComponent(recordingId)}`,
      { method: 'DELETE' }
    );
  } catch (error) {
    if (error instanceof TelnyxRequestError && error.status === 404) return;
    throw error;
  }
}

export async function getTelnyxCallRecording(recordingId: string): Promise<TelnyxRecordingResult> {
  const data = await telnyxFetch<Record<string, unknown>>(
    `/recordings/${encodeURIComponent(recordingId)}`
  );
  return parseTelnyxRecording(data, recordingId);
}

export async function findTelnyxCallRecording(params: {
  callControlId?: string | null;
  callLegId?: string | null;
  callSessionId?: string | null;
}): Promise<TelnyxRecordingResult | null> {
  const search = new URLSearchParams({ 'page[size]': '1', 'page[number]': '1' });
  if (params.callControlId) search.set('filter[call_control_id]', params.callControlId);
  if (params.callLegId) search.set('filter[call_leg_id]', params.callLegId);
  if (params.callSessionId) search.set('filter[call_session_id]', params.callSessionId);
  if (search.size === 2) return null;

  const recordings = await telnyxFetch<Array<Record<string, unknown>>>(`/recordings?${search.toString()}`);
  const recording = recordings[0];
  return recording ? parseTelnyxRecording(recording) : null;
}
