...
export async function findSalespersonLeadMaster(
  admin: SupabaseAdmin,
  input: Pick<MasterLeadInput, 'workspaceId' | 'assignedUserId' | 'contactId' | 'diallerLeadId' | 'name' | 'phone' | 'email' | 'address' | 'source' | 'externalId'>
  ...
  const hasStableExternalIdentity = Boolean(source && externalId);
  const lookups = [
    ...
    hasStableExternalIdentity
      ? { column: 'source_external', value: `${source}|${externalId}` }
      : null,
    // A phone number alone is not a unique business identity: franchises and
    // multi-location contractors often share one number. When a provider gives
    // us a stable external ID (for example a Google Place ID), use that plus an
    // exact fingerprint and do not collapse otherwise distinct locations.
    !hasStableExternalIdentity && phoneE164 ? { column: 'phone_e164', value: phoneE164 } : null,
    !hasStableExternalIdentity && emailKey ? { column: 'email_normalized', value: emailKey } : null,
...
function callDispositionToPipelineStage(disposition: DialerCallDisposition): 'connected' | 'lost' | null {
  switch (disposition) {
    case 'interested':
    case 'connected':
    case 'appointment_set':
...
export async function updateMasterLeadDispositionForDiallerLead(params: {
  ...
  const existing = await findSalespersonLeadMaster(params.admin, {
    assignedUserId: params.lead.user_id,
...
export async function updateMasterLeadDispositionForCall(params: {
  ...
  const pipelineStage = callDispositionToPipelineStage(params.disposition);
  ...
  if (pipelineStage) {
    const { data } = await params.admin
      ...
      .eq('stage_key', pipelineStage)
      .eq('is_archived', false)
      .order('position', { ascending: true })
      .limit(1)
      .maybeSingle();
    pipelineStageId = data?.id ? String(data.id) : null;
  }
