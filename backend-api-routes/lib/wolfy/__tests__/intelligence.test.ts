...
test('keeps useful AI advice without accepting an invented count',()=>{
 ...
 const r=recoverGroundedReply(JSON.stringify({message:'You made 999 sales. Review your overdue follow-ups before starting another session.',evidence_ids:['scope.current.overdue_tasks']}),e,'chat');
...
test('conversational chat does not require an unrelated performance statistic',()=>{
 const e=selectEvidence(analyze(fixture()),'AI working?','chat');
 for(const message of ["Yes, I can respond here. What would you like help with?", "What would you like me to clarify about my previous reply?"]){
  const reply=groundedReply(JSON.stringify({message,evidence_ids:[]}),e,'chat');
  assert.equal(reply.message,message);assert.deepEqual(reply.evidence,[]);
  assert.throws(()=>groundedReply(JSON.stringify({message,evidence_ids:[]}),e,'brief'));
 }
 assert.throws(()=>groundedReply(JSON.stringify({message:'You visited 42 doors today.',evidence_ids:[]}),e,'chat'));
 for(const question of ['?', 'Ai working']){
  const reply=fallbackReply(analyze(fixture()),question,'chat');
  assert.match(reply.message,/AI coaching is temporarily unavailable/);
  assert.deepEqual(reply.evidence,[]);assert.doesNotMatch(reply.message,/Appointments|Available context/);
 }
});

test('repair never attaches the first unrelated fact or accepts unknown evidence',()=>{
 const e=selectEvidence(analyze(fixture()),'AI working?','chat');
 const message='You made 999 sales. Review your overdue follow-ups before starting another session.';
 assert.throws(()=>recoverGroundedReply(JSON.stringify({message}),e,'chat'));
 assert.throws(()=>recoverGroundedReply(JSON.stringify({message,evidence_ids:['fake']}),e,'chat'));
 const reply=recoverGroundedReply(JSON.stringify({message,evidence_ids:['scope.current.overdue_tasks']}),e,'chat');
 assert.equal(reply.message,'Review your overdue follow-ups before starting another session.');
 assert.deepEqual(reply.evidence.map(f=>f.id),['scope.current.overdue_tasks']);
});
