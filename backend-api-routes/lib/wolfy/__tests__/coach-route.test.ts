...
test('field coaching route verifies scopes, reports without paid calls, caches, budget and evidence',async()=>{
 const original=globalThis.fetch;let paid=0,reserves=0,member=true,role='owner',allowed=true,badOutput=false,contextError=false,providerFailure=false,authFailure=false,cacheFailure=false,repairable=false,conversational=false;
 ...
 globalThis.fetch=async(input,init)=>{
  ...
  if(url.includes('/responses')){
   ...
   const e=JSON.parse(b.input).verifiedEvidence[0];
   if(conversational){
    assert.match(b.instructions,/empty evidence_ids array/);
    assert(['?', 'Ai working'].includes(JSON.parse(b.input).untrustedQuestion));
    return response({id:'resp_chat',object:'response',status:'completed',output:[{type:'message',role:'assistant',content:[{type:'output_text',text:JSON.stringify({message:'I can respond here. What would you like me to clarify?',evidence_ids:[]}),annotations:[]}]}]});
   }
   const message=badOutput||repairable?'You have 999 leads.':`[[${e.id}]] Review your next step before another session.`;
 ...
 try{
  ...
  conversational=true;
  for(const message of ['?', 'Ai working']){
   const chat=await(await POST(request({workspaceId:workspace,timezone:'UTC',mode:'chat',message}))).json();
   assert.equal(chat.source,'ai');assert.deepEqual(chat.evidence,[]);assert.doesNotMatch(chat.message,/Appointments|leads/);
  }
  const afterChat=paid;
  delete process.env.OPENAI_API_KEY;assert.equal((await(await POST(request())).json()).source,'rules');assert.equal(paid,afterChat);
  const unavailable=await(await POST(request({workspaceId:workspace,timezone:'UTC',mode:'chat',message:'Ai working'}))).json();
  assert.equal(unavailable.source,'rules');assert.match(unavailable.message,/unavailable/);assert.deepEqual(unavailable.evidence,[]);
