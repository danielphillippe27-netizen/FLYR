...
export function groundedReply(raw:string,evidence:Fact[],mode:string){
 ...
 if(!used.length&&mode!=='chat')throw Error('Missing evidence');
...
export function fallbackReply(a:Analysis,question:string,mode:string) {
 ...
 if(!parts.length){
  if(mode==='brief'){
   const selected=selectEvidence(a,question,mode).slice(0,1);
...
export function recoverGroundedReply(raw:string,evidence:Fact[],mode:string) {
 ...
 // Preserve the model's selected evidence; never insert an unrelated first fact.
 // Without selected evidence, stripping claims could disguise an invalid answer.
 if(!message.includes('[[')&&!parsed.evidence_ids?.length)throw Error('Missing evidence');
 return groundedReply(JSON.stringify({message,evidence_ids:parsed.evidence_ids??[]}),evidence,mode);
}
