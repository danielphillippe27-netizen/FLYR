...
const MAX_GOOGLE_PLACES_PAGES = 3;
...
async function callLegacyPlacesSearch(
  ...
  const results: LegacyPlacesTextSearchPlace[] = [];
  let pageToken: string | undefined;

  for (let page = 0; page < MAX_GOOGLE_PLACES_PAGES; page += 1) {
    // Legacy page tokens can take a few seconds to become active. Retrying an
    // INVALID_REQUEST token avoids silently losing pages two and three.
    let payload: LegacyPlacesResponse<LegacyPlacesTextSearchPlace> = {};
    for (let attempt = 0; attempt < (pageToken ? 3 : 1); attempt += 1) {
      ...
      payload = (await response.json().catch(() => ({}))) as LegacyPlacesResponse<LegacyPlacesTextSearchPlace>;
      ...
      if (payload.status !== 'INVALID_REQUEST' || !pageToken || attempt === 2) break;
    }

    assertLegacyPlacesOk(payload, 'Google Places text search request failed.');
    results.push(...(payload.results ?? []).slice(0, pageSize));
    pageToken = payload.next_page_token?.trim() || undefined;
    if (!pageToken) break;
  }

  const withDetails = await Promise.all(results.map((place) => callLegacyPlaceDetails(options, query, place)));
...
async function callPlacesSearch(
  ...
  const baseBody: Record<string, unknown> = {
    ...
    // Roofers and many other contractors often hide their street address and
    // serve a radius. They are still valid leads and should not be omitted.
    includePureServiceAreaBusinesses: true,
  };

  const countryCode = options.countryCode?.trim().toUpperCase();
  if (countryCode) baseBody.regionCode = countryCode;
  ...
  // Text Search (New) returns at most 20 places per page and at most 60 across
  // all pages. Fetch every available page instead of discarding pages 2 and 3.
  for (let page = 0; page < MAX_GOOGLE_PLACES_PAGES; page += 1) {
    ...
    const payload = (await response.json().catch(() => ({}))) as PlacesApiResponse & PlacesApiErrorPayload;
