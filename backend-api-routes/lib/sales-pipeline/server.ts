...
async function findLeadForDemoLink(
  ...
  const {data:salesperson,error:ownerError}=await admin.from('salespeople').select('user_id').eq('id',link.salesperson_id).eq('workspace_id',link.workspace_id).maybeSingle();
  if (ownerError || !salesperson?.user_id) return null;
  ...
  const { data, error } = await admin
    ...
    .eq('assigned_user_id',salesperson.user_id)
    .or(clauses.join(','))
...
async function insertPipelineActivity(params: {
  admin: SupabaseAdmin;
  lead: Pick<SalesLead, 'id' | 'workspace_id' | 'assigned_user_id' | 'assigned_salesperson_id' | 'assigned_sales_rep_id'>;
  ...
  const { error } = await params.admin
    ...
    .insert({
      ...
      actor_user_id: params.lead.assigned_user_id,
...
export async function recordDemoOpenInPipeline(params: {
  ...
  const { error } = await params.admin
    ...
    .eq('id', lead.id).eq('workspace_id',lead.workspace_id).eq('assigned_user_id',lead.assigned_user_id);
...
async function applyStrongSignupMatch(params: {
  ...
  const { error } = await params.admin
    ...
    .eq('id', params.lead.id).eq('workspace_id',params.lead.workspace_id).eq('assigned_user_id',params.lead.assigned_user_id);
...
async function findEmailMatchCandidates(params: {
  ...
  if (!params.salesperson.workspace_id || !params.salesperson.user_id) return [];

  let query = params.admin
    .from('sales_leads')
    .select(PIPELINE_SELECT)
    .eq('workspace_id', params.salesperson.workspace_id)
    .eq('assigned_user_id',params.salesperson.user_id)
