...
function normalizedTypeValues(value: BuildingLike | FeatureLike): string[] {
  ...
  return [
    ...
    stringValue(source.feature_type),
...
export function hasResidentialBuildingType(value: BuildingLike | FeatureLike): boolean {
  return normalizedTypeValues(value).some((type) => RESIDENTIAL_BUILDING_TYPES.has(type));
}
