...
export function getWebSuccessUrl(origin?: string): string {
  ...
  const appOrigin = origin || process.env.NEXT_PUBLIC_APP_URL || "https://www.flyrpro.app";
  return `${appOrigin.replace(/\/$/, "")}/integrations?fub=connected`;
}
...
export function getWebErrorUrl(origin?: string): string {
  ...
  const appOrigin = origin || process.env.NEXT_PUBLIC_APP_URL || "https://www.flyrpro.app";
  return `${appOrigin.replace(/\/$/, "")}/integrations?fub=error`;
}
...
export function createOAuthState(userId: string, platform: OAuthPlatform): string {
  ensureOAuthConfig();
