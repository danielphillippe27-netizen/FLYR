...
import { syncSalespersonReferralForSubscription } from '@/app/lib/billing/salespeople';
...
export async function applyStripeSubscriptionUpdate(
  ...
  const salespersonReferral = await syncSalespersonReferralForSubscription(
    supabase,
    userId,
    subscription
  );
  if (salespersonReferral?.salesperson_id) {
    try {
      const { data: salesperson } = await supabase
        .from('salespeople')
        .select('user_id')
        .eq('id', salespersonReferral.salesperson_id)
        .maybeSingle();
      if (salesperson?.user_id) {
        const revenue = await loadSalespersonStripeRevenue(
          supabase,
          salespersonReferral.salesperson_id,
          salesperson.user_id
        );
        await saveRevenueSnapshot(supabase, {
          salespersonId: salespersonReferral.salesperson_id,
          userId: salesperson.user_id,
          revenue,
        });
      }
    } catch (snapshotError) {
      console.warn('[stripe-subscription-sync] salesperson snapshot failed', {
