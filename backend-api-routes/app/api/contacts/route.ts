...
function isMissingRelation(error: { message?: string; details?: string | null }, relation: string) {
  ...
  return text.includes(relation.toLowerCase()) && (text.includes('relation') || text.includes('table')) && !text.includes('column') && (text.includes('does not exist') || text.includes('not find'));
}
...
export async function GET(request: NextRequest) {
  ...
  let query = admin
    .from('contacts')
    .select('id,user_id,workspace_id,full_name,email,phone,address,notes,tags,created_at,updated_at')
    .order('created_at', { ascending: false })
    .limit(200);
  if (requestedWorkspaceId && workspace.error) return NextResponse.json({ error: workspace.error }, { status: workspace.status ?? 403 });
  query = query.eq('user_id', requestUser.id);
...
export async function POST(request: NextRequest) {
  ...
  if (addressId) {
    const { data, error } = await admin
      ...
      .select('gers_id, campaign_id, address, campaigns!inner(owner_id,workspace_id)')
      .eq('id', addressId)
      .eq('campaigns.owner_id', requestUser.id)
      .eq('campaigns.workspace_id', workspace.workspaceId ?? '00000000-0000-0000-0000-000000000000')
      .maybeSingle();
    if (error) {
      return NextResponse.json({ error: `Failed to fetch linked address: ${error.message}` }, { status: 500 });
    }
    if (!data) return NextResponse.json({ error: 'Linked address not found.' }, { status: 404 });
    linkedAddress = data;
  }

  for (const [table, id] of [
    ['campaigns', getString(body.campaign_id) ?? linkedAddress?.campaign_id],
    ['farms', getString(body.farm_id)],
  ] as const) {
    if (!id) continue;
    let linkedQuery = admin.from(table).select('id').eq('id', id).eq('owner_id', requestUser.id);
    linkedQuery = workspace.workspaceId ? linkedQuery.eq('workspace_id', workspace.workspaceId) : linkedQuery.is('workspace_id', null);
    const { data: linked, error: linkedError } = await linkedQuery.maybeSingle();
    if (linkedError) return NextResponse.json({ error: 'Unable to validate linked record.' }, { status: 500 });
    if (!linked) return NextResponse.json({ error: 'Linked record not found.' }, { status: 404 });
  }
