...
async function sendText(
  ...
  try {
    ...
    const fromNumber = await getSalespersonSmsFromNumber(auth.admin, { userId: auth.requestUser.id, workspaceId: auth.workspaceId }, null);
    if (!fromNumber) return NextResponse.json({ error: 'A personal phone number must be assigned before texting.' }, { status: 409 });
    ...
    const [{ data: followup, error: insertError }, { error: activityError }, { error: contactError }] = await Promise.all([
      ...
      auth.admin.from('contact_activities').insert({
        communication_owner_user_id: auth.requestUser.id,
        contact_id: auth.contact.id,
        type: 'text',
...
async function sendEmail(
  ...
  try {
    ...
    const content = brandedEmailContent(body, auth.requestUser.email ?? from);
