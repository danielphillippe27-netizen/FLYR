...
export async function GET(request: Request) {
  try {
    ...
    const redirectUri = getMondayRedirectUri(url.origin);
    const state = createMondaySignedState(user.id, platform, workspaceId);
    ...
    const message = error instanceof Error ? error.message : "Unable to start Monday.com OAuth.";
