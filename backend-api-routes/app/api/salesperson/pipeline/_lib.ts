...
export const PIPELINE_LEAD_SELECT = `
  ...
  pipeline_stage,
  pipeline_stage_id,
  sales_pipeline_stages(id,stage_key,name,color,position,terminal_kind,is_archived),
  pipeline_owner_id,
  pipeline_priority,
  seat_count,
...
async function resolveSalesperson(
  ...
  const { data, error } = await admin.from('salespeople')
    .select('id,user_id,full_name,email,workspace_id').eq('user_id', user.id)
...
export async function loadLeadActivitiesAndMatches(params: {
  ...
  const [activitiesResult, matchesResult] = await Promise.all([
    params.admin
      ...
      .select('*')
      .eq('workspace_id', params.workspaceId).eq('actor_user_id', params.userId)
