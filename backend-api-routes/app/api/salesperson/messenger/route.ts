...
async function resolveActiveSalesperson(
  ...
  const { data, error } = await admin
    .from('salespeople')
    .select('id, user_id, full_name, email, workspace_id, founder_user_id')
    .eq('user_id', user.id)
...
async function loadSalespersonMessengerRecipients(
  ...
  const salespersonUserIds = Array.from(
    new Set(salespeople.map((row) => row.user_id).filter((id): id is string => Boolean(id)))
  );
  ...
  const [salespersonProfilesResult, founderProfilesResult] = await Promise.all([
    salespersonUserIds.length
      ? admin.from('profiles').select('id, email, full_name').in('id', salespersonUserIds)
  ...
  const salespersonProfileByUserId = new Map(
    ((salespersonProfilesResult.data ?? []) as RecipientProfileRow[]).map((profile) => [profile.id, profile])
  );
  ...
  for (const salesperson of salespeople) {
    const profile = salesperson.user_id ? salespersonProfileByUserId.get(salesperson.user_id) : null;
