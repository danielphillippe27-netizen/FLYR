...
import { resolveCompanyForLead } from '@/lib/company-research/identity';
...
function scraperLeadKey(lead: PlacesLead, phoneMarket: SupportedPhoneMarket = 'US'): string {
  // Google Place IDs distinguish real branches that may share a call-centre
  // phone number. Fall back to an exact business fingerprint only when Google
  // did not return a Place ID.
  return lead.placeId
    ? `place:${lead.placeId}`
    : `fingerprint:${contactSignature(lead, phoneMarket)}`;
}
...
async function saveScraperResults(params: {
  ...
  await forEachWithConcurrency(uniqueLeads.values(), 8, async (lead) => {
  ...
  await forEachWithConcurrency(leadsToImport, 8, async (lead) => {
    const leadKey = scraperLeadKey(lead, phoneMarket);
  ...
  const masterLeadIds = Array.from(masterIdByLeadKey.values());
  if (contactIds.size > 0 || masterLeadIds.length > 0) {
  ...
  if (masterMetadataById.size > 0) {
    await forEachWithConcurrency(
      masterMetadataById.entries(),
      8,
      async ([masterId, metadata]) => {
    ...
    // Canonicalize the imported businesses once so web, iOS, lists, the dialler,
    // and company research all point at the same sales_companies record.
    await forEachWithConcurrency(
      masterMetadataById.keys(),
      5,
      async (masterId) => {
        try {
          await resolveCompanyForLead(params.admin, workspaceId, masterId, params.userId);
        } catch (error) {
          console.warn('[salesperson/google-places] company identity resolution failed', { masterId, error });
        }
      }
    );
  }
