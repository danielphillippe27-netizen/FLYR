...
async function loadLeadWithContact(
  ...
  const [{ data: contact, error: contactError }, { data: activeCall, error: callError }] = await Promise.all([
    admin.from('contacts').select('*').eq('id', lead.contact_id).eq('workspace_id', workspaceId).eq('user_id', userId).single(),
    admin
      .from('dialer_calls')
      .select('*')
      .eq('session_lead_id', lead.id).eq('workspace_id', workspaceId).eq('user_id', userId)
...
export async function POST(
  ...
  if (currentLead && body.skipCurrent) {
    ...
    return NextResponse.json(await loadLeadWithContact(context.admin, currentLead as DialerSessionLead, context.workspaceId, context.requestUser.id));
  }
  ...
  return NextResponse.json(await loadLeadWithContact(context.admin, claimedLead as DialerSessionLead, context.workspaceId, context.requestUser.id));
}
