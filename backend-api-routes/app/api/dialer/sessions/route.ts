...
function summarizeSession(leads: DialerSessionLead[], calls: DialerCall[]) {
  ...
  const connected = calls.filter((call) => call.disposition === 'interested' || call.disposition === 'connected' || call.disposition === 'appointment_set').length;
  return {
    total: leads.length,
    pending,
    completed,
...
async function buildSessionResponse(
  ...
  if (contactIds.length > 0) {
    const { data: contactRows, error: contactsError } = await admin
      .from('contacts')
      .select('*')
      .eq('user_id', session.user_id).eq('workspace_id', session.workspace_id)
      .in('id', contactIds);
