...
import { signPersonalVoicemailDrop } from '@/lib/dialer/voicemail-drops';
...
async function setActiveDrop(
  ...
  const { data: owned, error: lookupError } = await admin.from('dialer_voicemail_drops').select('id').eq('workspace_id', workspaceId).eq('user_id', userId).eq('id', id).maybeSingle();
  if (lookupError || !owned) return { data: null, error: lookupError };
...
export async function GET(request: NextRequest) {
  ...
  const recordings = await Promise.all(((data ?? []) as DialerVoicemailDrop[]).map(row => signPersonalVoicemailDrop(context.admin, row, context.workspaceId, context.requestUser.id)));
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ recording: await signPersonalVoicemailDrop(context.admin, data as DialerVoicemailDrop, context.workspaceId, context.requestUser.id) });
}
...
export async function PATCH(request: NextRequest) {
  ...
  const { data, error } = await setActiveDrop(context.admin, context.workspaceId, context.requestUser.id, body.id);
  if (error || !data) {
