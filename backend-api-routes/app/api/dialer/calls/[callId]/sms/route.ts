...
export async function POST(
  ...
  try {
    ...
    const fromNumber = await getSalespersonSmsFromNumber(context.admin, { userId: context.requestUser.id, workspaceId: context.workspaceId }, null);
    ...
    const [{ data: followup, error: insertError }, { error: activityError }, { error: contactError }] = await Promise.all([
      ...
      contactId
        ? context.admin.from('contact_activities').insert({
            communication_owner_user_id: context.requestUser.id,
            contact_id: contactId,
            type: 'text',
