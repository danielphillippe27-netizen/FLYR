...
// Keep the web lead finder usable when prospect_industries has not been seeded.
// These are the same six choices and search terms used by the iOS Sales app.
const builtInIndustries: ProspectIndustry[] = [
  { id: 'built-in:roofing', slug: 'roofing', name: 'Roofing', default_terms: ['roofing company', 'roofer', 'roof repair', 'roof replacement'], priority: 1 },
  { id: 'built-in:solar', slug: 'solar', name: 'Solar', default_terms: ['solar company', 'solar installer', 'solar panels', 'residential solar'], priority: 2 },
  { id: 'built-in:real-estate', slug: 'real-estate', name: 'Real Estate', default_terms: ['real estate agent', 'realtor', 'real estate brokerage'], priority: 3 },
  { id: 'built-in:pest-control', slug: 'pest-control', name: 'Pest Control', default_terms: ['pest control', 'exterminator', 'termite control'], priority: 4 },
  { id: 'built-in:lawncare', slug: 'lawncare', name: 'Lawncare', default_terms: ['lawn care', 'lawn service', 'landscaping', 'yard maintenance'], priority: 5 },
  { id: 'built-in:home-security', slug: 'home-security', name: 'Home Security', default_terms: ['home security', 'security systems', 'alarm systems', 'security installer'], priority: 6 },
];
...
export function SalespersonPlacesLeadFinder() {
  ...
  const [industries, setIndustries] = useState<ProspectIndustry[]>(builtInIndustries);
  ...
  const loadProspectingOptions = useCallback(async () => {
    try {
      ...
      setIndustries(payload.industries?.length ? payload.industries : builtInIndustries);
      setRecentRuns(payload.recentRuns ?? []);
    } catch {
      setProspectingWorkspaceId(null);
      setMarkets([]);
      setIndustries(builtInIndustries);
      setRecentRuns([]);
    }
  }, [currentWorkspaceId]);
    ...
    try {
      ...
      const response = await fetch('/api/salesperson/google-places', {
        ...
        body: JSON.stringify({
          ...
          industryId: uuidPattern.test(selectedIndustryId) ? selectedIndustryId : undefined,
          leadSource: 'places',
          leadIntent,
        }),
      });
