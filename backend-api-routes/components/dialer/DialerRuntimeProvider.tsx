...
export function DialerRuntimeProvider({ children }: { children: ReactNode }) {
  ...
  const placeNextDiallerCall = useCallback(async (): Promise<{ ok: boolean; message: string }> => {
    ...
    if (device.isInCall) device.hangUp();
    setDiallerRunning(false);
    setActiveCallId(null);
    setActiveCallIsDoubleDial(false);
    setActiveLeadId(nextLead.id);
    return { ok: true, message: `${nextLead.name || 'Next lead'} ready. Press Dial to call.` };
  }, [activeCallId, activeLeadId, device, diallerLeads, markDiallerLeadCalled]);
  ...
  useEffect(() => {
    ...
    const inspectAmdStatus = async () => {
      try {
        ...
        if (leadId && isTerminalDialerCall(data.call) && getCallOutcome(data.call) !== 'pending') {
  ...
  }, [activeCallId, activeLeadId, currentWorkspaceId, device, diallerRunning, markDiallerLeadCalled]);
