...
actor FlyrCalendarService {
    ...
    private let appleCalendarDefaultsKey = "wolfgrid.calendar.appleCalendarPullEnabled"

    private init() {}

    func isAppleCalendarPullEnabled() async -> Bool {
        guard let userId = await currentUserId() else { return false }
        return UserDefaults.standard.bool(forKey: appleCalendarDefaultsKey(for: userId))
    }

    func setAppleCalendarPullEnabled(_ isEnabled: Bool) async {
        guard let userId = await currentUserId() else { return }
        UserDefaults.standard.set(isEnabled, forKey: appleCalendarDefaultsKey(for: userId))
    }
    ...
    func requestAppleCalendarPull() async -> AppleCalendarAccessState {
        ...
        await setAppleCalendarPullEnabled(state == .fullAccess)
        return state
    }
    ...
    private func fetchStandaloneEvents(userId: UUID, workspaceId: UUID?, start: Date, end: Date) async -> [FlyrCalendarEvent] {
        ...
        do {
            ...
            // Calendar is personal. workspace_id may describe context, but must
            // never expand visibility to teammates.
    ...
    private func fetchAppleCalendarItemsIfEnabled(start: Date, end: Date) async -> [CalendarItem] {
        guard await isAppleCalendarPullEnabled() else { return [] }
        let state = await AppleCalendarService.shared.currentAccessState()
        guard state == .fullAccess else {
            await setAppleCalendarPullEnabled(false)
            return []
        }
    ...
    private func currentUserId() async -> UUID? {
        await MainActor.run { AuthManager.shared.user?.id }
    }

    private func appleCalendarDefaultsKey(for userId: UUID) -> String {
        "\(appleCalendarDefaultsKey).\(userId.uuidString.lowercased())"
    }
