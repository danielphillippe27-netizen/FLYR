...
@MainActor struct WolfyLaboratoryEntryView: View {
    ...
    var body:some View {
        Group {
            if ProcessInfo.processInfo.arguments.contains("--wolfy-kpis"),
               let url = Bundle.main.url(forResource: "wolfy_kpi_fixture", withExtension: "json"),
               let data = try? Data(contentsOf: url), let report = try? JSONDecoder().decode(WolfyKPIReport.self, from: data) {
                WolfyKPIReportView(report: report)
            } else if ProcessInfo.processInfo.arguments.contains("--wolfy-coach") {
