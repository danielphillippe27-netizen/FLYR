...
actor FieldLeadsService {
    ...
    func fetchLeads(userId: UUID, workspaceId: UUID? = nil, campaignId: UUID? = nil, sessionId: UUID? = nil) async throws -> [FieldLead] {
        ...
        do {
            ...
            contactLeadCount = contacts.count
            // A successful owner-scoped response is authoritative. Older app
            // versions could cache workspace-visible rows under the current
            // user's ID, so merging that cache would keep exposing those rows
            // even after the server RLS policy was corrected.
            mergedLeads.removeAll(keepingCapacity: true)
            if !contacts.isEmpty {
        ...
        // `field_leads` is retained only for installations that have not yet
        // consolidated into `contacts`. Avoid a second network round-trip for
        // every normal inbox load.
        if !didLoadContacts || (contactLeadCount == 0 && mergedLeads.isEmpty) {
            do {
