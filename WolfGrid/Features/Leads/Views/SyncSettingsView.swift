...
struct SyncSettingsView: View {
    ...
    @State private var catalogProviders: [IntegrationCatalogProvider] = []
    ...
    private var connectedProviders: [IntegrationProvider] {
        ...
        for entry in catalogProviders where entry.connected {
            if let provider = entry.nativeProvider, !providers.contains(provider) {
    ...
    /// Keep the two primary CRM options available even when an older provider
    /// catalog is returned by the API. CompanyCam is intentionally hidden until
    /// that integration is ready to be offered in the mobile apps.
    private var quickConnectProviders: [IntegrationProvider] {
        let catalog = catalogProviders.compactMap(\.nativeProvider)
            .filter { $0 != .companycam }
        return ([.fub, .hubspot] + catalog).reduce(into: []) { providers, provider in
    ...
    private var quickConnectSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Quick Connect")
                ...
                .foregroundColor(.muted)
            VStack(spacing: 12) {
                ForEach(quickConnectProviders, id: \.self) { provider in
                    ...
                    let isConnected: Bool = {
                        ...
                        default:
                            return catalogProviders.first { $0.nativeProvider == provider }?.connected
    ...
    private func loadIntegrations() async {
        ...
        do {
            ...
            async let catalogTask = CRMIntegrationManager.shared.fetchProviderCatalog()
            async let crmTask: Void = CRMConnectionStore.shared.refresh(userId: userId)
            integrations = try await integrationsTask
            catalogProviders = try await catalogTask
            await crmTask
    ...
    private func connectWithAPIKey() {
        ...
        Task {
            do {
                ...
                default:
                    try await CRMIntegrationManager.shared.connectCatalogProvider(provider, credential: apiKeyText)
                }
                await MainActor.run {
    ...
    private func makeGenericTestLead(source: String, notes: String) -> LeadModel {
        ...
        return LeadModel(
            name: "WolfGrid Test Lead",
            phone: "5555555555",
            email: "test+\(timestamp)@wolfgrid.app",
        ...
        do {
            ...
            case .fub:
                ...
                let response = try await FUBPushLeadAPI.shared.pushLead(
                    makeGenericTestLead(
                        source: "WolfGrid iOS Follow Up Boss Test",
            ...
            case .boldtrail:
                ...
                _ = try await BoldTrailPushLeadAPI.shared.pushLead(
                    makeGenericTestLead(
                        source: "WolfGrid iOS BoldTrail Test",
            ...
            case .hubspot:
                ...
                let appointment = LeadSyncAppointment(
                    date: Date().addingTimeInterval(2 * 60 * 60),
                    title: "WolfGrid HubSpot Test Appointment",
                    notes: "Test appointment from Sync Settings."
                )
                let task = LeadSyncTask(
                    title: "WolfGrid HubSpot Test Follow Up",
                    dueDate: Date().addingTimeInterval(24 * 60 * 60)
                )
                let hubRes = try await HubSpotPushLeadAPI.shared.pushLead(
                    makeGenericTestLead(
                        source: "WolfGrid iOS HubSpot Test",
                        notes: "Test note from Sync Settings. Please verify notes sync."
                    ),
                    appointment: appointment,
                    task: task
