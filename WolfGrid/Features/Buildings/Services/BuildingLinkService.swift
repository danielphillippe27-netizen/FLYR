...
struct CanonicalCampaignMapBundleCounts: Codable, Sendable {
    ...
    init(
        ...
        self.buildingUnits = buildingUnits
    }
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case roads
        case links
        case buildingUnits = "building_units"
        case renderVersion = "render_version"
    }
}

struct CanonicalMapReconciliation: Codable, Sendable {
    let status: String
    let runId: String?
    ...
    let appliedBundleSignature: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case appliedBundleSignature = "applied_bundle_signature"
        case report
    }
...
struct CanonicalMapReconciliationReport: Codable, Sendable {
    ...
    let labelAnchorsAdjusted: Int?
    let syntheticAddressesCreated: Int?
    let duplicateBuildingsHidden: Int?
    let auxiliaryBuildingsHidden: Int?
    let addressOrphansBefore: Int?
    let addressOrphansAfter: Int?
    let buildingOrphansBefore: Int?
    let buildingOrphansAfter: Int?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case labelAnchorsAdjusted = "label_anchors_adjusted"
        case syntheticAddressesCreated = "synthetic_addresses_created"
        case duplicateBuildingsHidden = "duplicate_buildings_hidden"
        case auxiliaryBuildingsHidden = "auxiliary_buildings_hidden"
        case addressOrphansBefore = "address_orphans_before"
        case addressOrphansAfter = "address_orphans_after"
        case buildingOrphansBefore = "building_orphans_before"
        case buildingOrphansAfter = "building_orphans_after"
        case coverageBefore = "coverage_before"
        case coverageAfter = "coverage_after"
...
enum MapQualityReportHistoryStore {
    private static func key(campaignId: String) -> String {
        "wolfgrid.map-quality-history.\(campaignId.lowercased())"
    }

    static func reports(campaignId: String) -> [CachedMapQualityReport] {
        guard let data = UserDefaults.standard.data(forKey: key(campaignId: campaignId)),
              let reports = try? JSONDecoder().decode([CachedMapQualityReport].self, from: data) else {
            return []
        }
        return reports.sorted { $0.downloadedAt > $1.downloadedAt }
    }

    static func save(
        campaignId: String,
        runId: String,
        report: CanonicalMapReconciliationReport
    ) {
        var reports = reports(campaignId: campaignId)
            .filter { $0.runId.caseInsensitiveCompare(runId) != .orderedSame }
        reports.insert(
            CachedMapQualityReport(runId: runId, downloadedAt: Date(), report: report),
            at: 0
        )
        reports = Array(reports.prefix(12))
        if let data = try? JSONEncoder().encode(reports) {
            UserDefaults.standard.set(data, forKey: key(campaignId: campaignId))
        }
    }
}

struct CanonicalTownhouseUnit: Codable, Sendable {
    let id: String
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case splitVersion = "split_version"
        case splitSignature = "split_signature"
...
struct CanonicalCampaignMapBundle: Codable, Sendable {
    ...
    let reconciliationReport: CanonicalMapReconciliationReport?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case roads
        case links
        case addressOrphans = "address_orphans"
        case buildingOrphans = "building_orphans"
        case buildingUnits = "building_units"
        case reconciliation
...
final class BuildingLinkService {
    ...
    func performRemoteLinkAddressToBuilding(
        ...
        baseBundleSignature: String? = nil,
        offlineMutation: Bool = false
    ) async throws -> BuildingAddressMutationResponse {
        ...
        do {
            return try await postLinkAddressToBuilding(
                ...
                baseBundleSignature: baseBundleSignature,
    ...
    private func postLinkAddressToBuilding(
        ...
        baseBundleSignature: String?,
        offlineMutation: Bool
    ) async throws -> BuildingAddressMutationResponse {
        let encodedCampaignId = encodedPathComponent(campaignId)
        let encodedBuildingId = encodedPathComponent(buildingId)
        let usesResolutionContract = baseBundleSignature?.isEmpty == false
        let endpoint = usesResolutionContract ? "address-resolution" : "addresses"
        guard let url = URL(string: "\(baseURL)/api/campaigns/\(encodedCampaignId)/buildings/\(encodedBuildingId)/\(endpoint)") else {
            throw BuildingLinkError.invalidURL
        }

        var payload: [String: Any] = ["address_id": addressId.uuidString]
        let mutationKey = idempotencyKey ?? UUID().uuidString
        if usesResolutionContract {
            payload["action"] = "confirm_existing"
            payload["base_bundle_signature"] = baseBundleSignature
            payload["idempotency_key"] = mutationKey
            payload["offline_mutation"] = offlineMutation
        }
        if let coordinate {
        ...
        let (responseData, response) = try await authorizedDataRequest(
            url: url,
            method: "POST",
            body: data,
            additionalHeaders: usesResolutionContract
    ...
    private func unlinkAddressFromBuilding(
        ...
        do {
            let baseBundleSignature = await campaignRepository
                .getCampaignMapBundleMetadata(campaignId: campaignId)?
                .assetSignature
    ...
    func performRemoteUnlinkAddressFromBuilding(
        ...
        let encodedCampaignId = encodedPathComponent(campaignId)
        let encodedBuildingId = encodedPathComponent(buildingId)
        if let baseBundleSignature, !baseBundleSignature.isEmpty {
            guard let url = URL(
                string: "\(baseURL)/api/campaigns/\(encodedCampaignId)/buildings/\(encodedBuildingId)/address-resolution"
            ) else {
                throw BuildingLinkError.invalidURL
            }
            let mutationKey = idempotencyKey ?? UUID().uuidString
            let payload: [String: Any] = [
                ...
                "delete_manual_address": deleteManualAddress,
                "base_bundle_signature": baseBundleSignature,
            ...
            let (data, response) = try await authorizedDataRequest(
                ...
                method: "POST",
                body: body,
                additionalHeaders: ["Idempotency-Key": mutationKey]
            )
...
struct BuildingAddressCandidate: Identifiable, Decodable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case rank
        case tier
        case evidenceCodes = "evidence_codes"
    ...
    init(from decoder: Decoder) throws {
        ...
        evidenceCodes = (try? container.decodeIfPresent([String].self, forKey: .evidenceCodes)) ?? []
