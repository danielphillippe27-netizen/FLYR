...
// #region agent log
#if DEBUG
private let _debugLogDoorsQueue = DispatchQueue(label: "com.wolfgrid.debuglog", qos: .background)
...
class SessionManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private var manualPinTargetAddressIds: Set<String> = []
    ...
    var targetCount: Int { targetBuildings.count + manualPinTargetAddressIds.count }
    ...
    func recordManualPinTarget(addressId: UUID) {
        guard sessionId != nil, !isNetworkingSession else { return }
        let key = normalizeAddressKey(addressId)
        guard !manualPinTargetAddressIds.contains(key) else { return }

        manualPinTargetAddressIds.insert(key)
        if sessionMode == .doorKnocking, goalType == .knocks {
            goalAmount += 1
        }

        let updatedGoalAmount = goalAmount
        Task { [weak self] in
            guard let self else { return }
            if let sessionId = self.sessionId {
                await self.sessionRepository.updateSessionGoalAmount(id: sessionId, goalAmount: updatedGoalAmount)
            }
            await self.persistLocalActiveSessionSnapshot()
            await self.syncLiveActivity(forceStart: false)
        }
    }
    ...
    func startBuildingSession(
        ...
        // Publish active state after the local session exists. Remote create and
        // participant registration continue below so network latency cannot hold
        // the Start button in its loading state.
        self.campaignId = campaignId
        sessionId = newSessionId
        activeSharedLiveSessionId = nil
        lastSharedLivePresencePeriodicSync = nil
        ...
        Task { [weak self] in
            // Let the caller observe the active session before non-critical network work
            // such as Beacon restore or session event enqueueing resumes on MainActor.
            try? await Task.sleep(for: .milliseconds(150))
            await self?.syncStartedSessionAfterActivation(
                campaignId: campaignId,
                sessionId: newSessionId,
                userId: userId,
                targetBuildings: targetBuildings,
    ...
    private func syncStartedSessionAfterActivation(
        campaignId: UUID,
        sessionId: UUID,
        userId: UUID,
        targetBuildings: [String],
        ...
        offlinePayload: OfflineSessionPayload
    ) async {
        if NetworkMonitor.shared.isOnline {
            do {
                ...
                logSessionStart(.createSession, "success session=\(sessionId.uuidString)")
            ...
            logSessionStart(.createSession, "queued for offline sync session=\(sessionId.uuidString)")
        }
    ...
    private func persistEndedBuildingSession(
        ...
        _ = await outboxRepository.enqueue(
            entityType: "session",
            entityId: sessionId.uuidString,
