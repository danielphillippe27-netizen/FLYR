...
final class SessionSafetyBeaconService: ObservableObject {
    ...
    static let defaultShareMessage = "Here's my live location for my next activity! Follow along on WolfGrid."
    ...
    private func rescheduleNotifications(for sessionId: UUID, from nextPromptAt: Date?) async {
        ...
        let promptRequest = UNNotificationRequest(
            ...
            content: notificationContent(
                ...
                body: "Still good? Open WolfGrid and confirm your Beacon check-in."
            ),
        ...
        let graceRequest = UNNotificationRequest(
            ...
            content: notificationContent(
                ...
                body: "Your safety check-in is overdue. Open WolfGrid to confirm you're okay."
            ),
    ...
    private func cancelNotifications(for sessionId: UUID) {
        UNUserNotificationCenter.current().removePendingNotificationRequests(withIdentifiers: [
            ...
            legacyPromptNotificationId(for: sessionId),
            legacyGraceNotificationId(for: sessionId),
        ])
    }

    private func promptNotificationId(for sessionId: UUID) -> String {
        "wolfgrid.beacon.prompt.\(sessionId.uuidString)"
    }

    private func graceNotificationId(for sessionId: UUID) -> String {
        "wolfgrid.beacon.grace.\(sessionId.uuidString)"
    }

    private func legacyPromptNotificationId(for sessionId: UUID) -> String {
        "flyr.beacon.prompt.\(sessionId.uuidString)"
    }

    private func legacyGraceNotificationId(for sessionId: UUID) -> String {
