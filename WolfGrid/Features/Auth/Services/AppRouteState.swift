...
final class AppRouteState: ObservableObject {
    ...
    func resolveRoute() async {
        ...
        #if DEBUG
        // Local E2E must stay on the isolated Supabase data plane. Resolving through the hosted
        // access API would mix environments and can incorrectly send a valid QA user to onboarding.
        if SupabaseManager.shared.isLocalE2E {
            if let workspaceID = SupabaseManager.shared.localE2EWorkspaceID {
                WorkspaceContext.shared.update(
                    workspaceId: workspaceID,
                    name: "Isolated E2E Workspace",
                    role: "member"
                )
                route = .dashboard
                return
            }
            route = await RoutePlansAPI.shared.existingWorkspaceIdForCurrentUser() == nil
                ? .onboarding
                : .dashboard
            return
        }
