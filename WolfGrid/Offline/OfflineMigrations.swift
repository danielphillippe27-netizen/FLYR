...
enum OfflineMigrations {
    static func migrator() -> DatabaseMigrator {
        ...
        migrator.registerMigration("campaign_map_reconciliation_cache_v1") { db in
            let existingColumns = Set(try db.columns(in: "cached_campaign_map_bundles").map(\.name))
            try db.alter(table: "cached_campaign_map_bundles") { table in
                if !existingColumns.contains("reconciliation_json") {
                    table.add(column: "reconciliation_json", .text)
                }
                if !existingColumns.contains("reconciliation_report_json") {
                    table.add(column: "reconciliation_report_json", .text)
                }
                if !existingColumns.contains("address_orphans_json") {
                    table.add(column: "address_orphans_json", .text)
                }
                if !existingColumns.contains("building_orphans_json") {
                    table.add(column: "building_orphans_json", .text)
                }
                if !existingColumns.contains("building_units_json") {
                    table.add(column: "building_units_json", .text)
                }
            }
        }
