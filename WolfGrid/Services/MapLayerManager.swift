...
/// Mirrors WolfGrid web's MapBuildingsLayer.tsx functionality
@MainActor
final class MapLayerManager {
    ...
    static let manualPinBaseLayerId = "campaign-manual-pins-base-layer"
    static let manualPinLayerId = "campaign-manual-pins-layer"
    ...
    private static var manualPinFilterExpression: Exp {
        Exp(.all) {
            Exp(.eq) {
                Exp(.geometryType)
                "Point"
    ...
    private static var manualPinMarkerExpression: Exp {
        Exp(.any) {
            Exp(.match) {
                Exp(.coalesce) {
                    Exp(.get) { "feature_type" }
                    Exp(.get) { "source" }
                    Exp(.get) { "address_provenance" }
                    ""
                }
                ["manual_pin", "field_manual_pin"]
                true
                false
            }
            Exp(.all) {
    ...
    private static func isManualPinMarkerProperties(_ properties: [String: Any]) -> Bool {
        let markerValues = [
            properties["feature_type"],
            properties["source"],
            properties["address_provenance"]
        ]
        ...
        let source = (properties["source"] as? String)?
            ...
            .lowercased()
        let hasBuildingLink = properties["has_building_link"] as? Bool
        let buildingIds = [
        ...
        let hasBuildingIdentifier = buildingIds.contains { value in
            guard let raw = value as? String else { return false }
            return !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        }
        let labelMode = (properties["label_visibility_mode"] as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        if source == "manual" &&
            hasBuildingLink != true &&
            !hasBuildingIdentifier &&
            labelMode == "address_mode_only" {
            return true
        }

        let formatted = (properties["formatted"] as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
    ...
    private static var manualPinModelColorExpression: Exp {
        Exp(.switchCase) {
    ...
    private static func manualPinModelURL(resourceName: String) -> URL? {
        Bundle.main.url(forResource: resourceName, withExtension: "glb", subdirectory: "MapAssets")
            ?? Bundle.main.url(forResource: resourceName, withExtension: "glb", subdirectory: "Resources/MapAssets")
            ?? Bundle.main.url(forResource: resourceName, withExtension: "glb")
    }
    ...
    func setupBuildingsLayer() {
        ...
        MapTheme.configureExtrusionDepth(&layer)
    ...
    func setupTownhomeStatusLayer() {
        ...
        layer.fillExtrusionOpacity = .constant(1.0)
        // Keep shared townhouse edges nearly square so neighboring status
        // segments do not reveal gaps, while still softening the outer silhouette.
        MapTheme.configureExtrusionDepth(&layer, edgeRadius: 0.12)
        layer.minZoom = Self.buildingLayerMinZoom
    ...
    func setupAddressesLayer() {
        ...
        layer.fillExtrusionColorTransition = StyleTransition(duration: 0.18, delay: 0)
        layer.fillExtrusionHeightTransition = StyleTransition(duration: 0.18, delay: 0)
        MapTheme.configureExtrusionDepth(
            &layer,
    ...
    func ensureManualPinLayers() {
        setupManualPinLayer()
    }

    private func setupManualPinLayer() {
        guard let mapView = mapView else { return }
        guard mapView.mapboxMap.sourceExists(withId: Self.addressesSourceId) else {
            print("⚠️ [MapLayer] Manual pin layers waiting for addresses source")
            return
        }

        guard let baseModelURL = Self.manualPinModelURL(resourceName: "PushPinBase"),
              let topModelURL = Self.manualPinModelURL(resourceName: "PushPinTop") else {
            print("❌ [MapLayer] Missing bundled split PushPin GLB assets for manual pin model layers")
            return
        }

        do {
            try mapView.mapboxMap.addStyleModel(modelId: Self.manualPinBaseModelId, modelUri: baseModelURL.absoluteString)
            try mapView.mapboxMap.addStyleModel(modelId: Self.manualPinTopModelId, modelUri: topModelURL.absoluteString)
        } catch {
            print("ℹ️ [MapLayer] Manual pin models already registered or unavailable to re-register: \(error)")
        }

        if mapView.mapboxMap.layerExists(withId: Self.manualPinBaseLayerId),
           mapView.mapboxMap.layerExists(withId: Self.manualPinLayerId) {
            print("🔍 [MapLayer] Manual pin model layers already present base=true top=true")
            return
        }

        if mapView.mapboxMap.layerExists(withId: Self.manualPinLayerId) {
            try? mapView.mapboxMap.removeLayer(withId: Self.manualPinLayerId)
        }
        if mapView.mapboxMap.layerExists(withId: Self.manualPinBaseLayerId) {
            try? mapView.mapboxMap.removeLayer(withId: Self.manualPinBaseLayerId)
        }

        func makePinLayer(id: String, modelId: String) -> ModelLayer {
            var layer = ModelLayer(id: id, source: Self.addressesSourceId)
            layer.modelId = .constant(modelId)
            ...
            layer.modelReceiveShadows = .constant(false)
            layer.minZoom = Self.addressMarkersLayerMinZoom
            layer.filter = Self.manualPinFilterExpression
            return layer
        }

        let baseLayer = makePinLayer(id: Self.manualPinBaseLayerId, modelId: Self.manualPinBaseModelId)
        var topLayer = makePinLayer(id: Self.manualPinLayerId, modelId: Self.manualPinTopModelId)
        topLayer.modelColor = .expression(Self.manualPinModelColorExpression)
        topLayer.modelColorMixIntensity = .constant(1.0)

        do {
            let layerIds = Set(mapView.mapboxMap.allLayerIdentifiers.map(\.id))
            if layerIds.contains(Self.selectedAddressesLayerId) {
                try mapView.mapboxMap.addLayer(baseLayer, layerPosition: .above(Self.selectedAddressesLayerId))
            } else if layerIds.contains(Self.addressesLayerId) {
                try mapView.mapboxMap.addLayer(baseLayer, layerPosition: .above(Self.addressesLayerId))
            } else {
                try mapView.mapboxMap.addLayer(baseLayer)
            }
            try mapView.mapboxMap.addLayer(topLayer, layerPosition: .above(Self.manualPinBaseLayerId))
            print("✅ [MapLayer] Added split manual pin model layers (\(Self.manualPinBaseLayerId), \(Self.manualPinLayerId))")
        } catch {
            print("❌ [MapLayer] Error adding manual pin model layers: \(error)")
        }
    }
    ...
    func setupLighting() {
        ...
        MapTheme.applyLightModeShadowPolicy(to: mapView.mapboxMap)
        print("✅ [MapLayer] Configured 3D lighting and native shadows")
    }
    ...
    func updateAddresses(
        ...
        do {
            ...
            let renderedFeatures = (try? JSONSerialization.jsonObject(with: polygonData) as? [String: Any])
                .flatMap { $0["features"] as? [[String: Any]] } ?? []
            let polygonCount = renderedFeatures.filter { feature in
                guard let geometry = feature["geometry"] as? [String: Any],
                      let type = geometry["type"] as? String else { return false }
                return type == "Polygon" || type == "MultiPolygon"
            }.count
            let manualPinPointCount = renderedFeatures.filter { feature in
                guard let geometry = feature["geometry"] as? [String: Any],
                      geometry["type"] as? String == "Point",
                      let properties = feature["properties"] as? [String: Any] else { return false }
                return Self.isManualPinMarkerProperties(properties)
            }.count
            print("🔍 [MapLayer] Address render source: \(pointCount) points → \(polygonCount) extrusion polygon(s), \(manualPinPointCount) manual pin model point(s)")
            if polygonCount == 0, manualPinPointCount == 0, pointCount > 0 {
            ...
            if manualPinPointCount > 0 {
                ensureManualPinLayers()
                print(
                    "🔍 [MapLayer] Manual pin layers presence base=\(mapView.mapboxMap.layerExists(withId: Self.manualPinBaseLayerId)) " +
                    "top=\(mapView.mapboxMap.layerExists(withId: Self.manualPinLayerId)) " +
                    "source=\(mapView.mapboxMap.sourceExists(withId: Self.addressesSourceId))"
                )
            }
            ...
            let renderedFeatureCount = renderedFeatures.count
            if renderedFeatureCount > 0 {
                print("✅ [MapLayer] Updated addresses source (\(Self.addressesSourceId)) features=\(renderedFeatureCount) polygons=\(polygonCount) manualPins=\(manualPinPointCount) (layer minZoom=\(Self.addressMarkersLayerMinZoom))")
            } else {
    ...
    private func applyAddressExtrusionVisibilityIfNeeded() {
        ...
        for layerId in [Self.manualPinBaseLayerId, Self.manualPinLayerId] where mapView.mapboxMap.layerExists(withId: layerId) {
            try? mapView.mapboxMap.updateLayer(withId: layerId, type: ModelLayer.self) {
    ...
    func updateAddressModeZoomVisibility(isAddressMode: Bool) {
        ...
        // House numbers become noisy and unreadable in an overview. Keep the same
        // close-range cutoff in every map mode, including address mode.
        ...
        for layerId in [Self.manualPinBaseLayerId, Self.manualPinLayerId] where map.layerExists(withId: layerId) {
            try? map.updateLayer(withId: layerId, type: ModelLayer.self) {
                $0.minZoom = addressMinZoom
            }
        }
    ...
    private static func smartAddressPointFeatures(
        ...
        addresses.compactMap { feature in
            ...
            let isManualPin = isManualPinAddressFeature(feature)
            ...
            if let featureType = feature.properties.featureType {
                labelProperties["feature_type"] = featureType
            } else if isManualPinAddressFeature(feature) {
                labelProperties["feature_type"] = "manual_pin"
            }
    ...
    private static func convertAddressPointsToCirclePolygons(_ pointGeoJSONData: Data, radiusMeters: Double = 2.7, height: Double = 10.8, segments: Int = 20) throws -> Data {
        ...
        for feature in features {
            ...
            let originalProps = (feature["properties"] as? [String: Any]) ?? [:]
            if isManualPinMarkerProperties(originalProps) {
                var manualPinFeature = feature
                var manualPinProperties = originalProps
                manualPinProperties["feature_type"] = "manual_pin"
                manualPinProperties["source"] = "manual_pin"
                manualPinProperties["label_visibility_mode"] = "address_mode_only"
                manualPinFeature["properties"] = manualPinProperties
                polygonFeatures.append(manualPinFeature)
                preservedManualPins += 1
                continue
            }
        ...
        if preservedManualPins > 0 {
            print("✅ [MapLayer] Preserved \(preservedManualPins) manual pin point feature(s) for 3D model layer")
        }
        let collection: [String: Any] = [
            "type": "FeatureCollection",
    ...
    func flyerProximityAddress(at coordinate: CLLocationCoordinate2D, searchMeters: Double) async -> FlyerAddress? {
        ...
        if let address = await queryFlyerAddress(
            ...
            layerIds: [
                VectorTileDiamondGeometryProvider.selectedAddressCircleLayerId,
                VectorTileDiamondGeometryProvider.addressCircleLayerId,
                Self.manualPinBaseLayerId,
                Self.manualPinLayerId,
