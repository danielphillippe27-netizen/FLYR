...
final class OvertureAddressService {
    ...
    private var baseURLFromPlist: Bool {
        (Bundle.main.object(forInfoDictionaryKey: "WOLFGRID_API_URL") as? String) != nil
    }
    ...
    private func logBackendDiagnostics(path: String, error: Error? = nil) {
        let connPath = "HTTP backend (WOLFGRID_API_URL)"
        ...
        if let e = error {
            ...
            if desc.contains("hostname") || desc.contains("could not be found") || desc.contains("cannot find") {
                print("💡 [ADDRESS BACKEND] Hostname/DNS failure: check WOLFGRID_API_URL in Info.plist / xcconfig (e.g. wolfgrid.app).")
            }
        }
    }
    ...
    /// Creates a short-lived web session cookie on wolfgrid.app from Supabase bearer token.
