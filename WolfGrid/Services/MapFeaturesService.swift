...
struct GeoJSONCoordinatesNode: Codable, Sendable {
    ...
    init(point: [Double]) {
        value = .array(point.map { coordinate in
            GeoJSONCoordinatesNode(number: coordinate)
        })
    }

    private init(number: Double) {
        value = .number(number)
    }
...
final class MapFeaturesService: ObservableObject {
    ...
    @Published var reconciliationStatus: CanonicalMapReconciliation?
    @Published var mapQualityReport: CanonicalMapReconciliationReport?
    @Published var mapQualityReportRunId: String?
    ...
    /// Campaigns whose feature request has produced a valid response. A valid response may
    /// legitimately contain no features, which is distinct from a request that has not run
    /// yet or one that failed.
    private var loadedCampaignIds: Set<String> = []
    ...
    func hasLoadedCampaignData(campaignId: String) -> Bool {
        loadedCampaignIds.contains(campaignId.lowercased())
    }

    private func markCampaignDataLoaded(campaignId: String) {
        loadedCampaignIds.insert(campaignId.lowercased())
    }
    ...
    func fetchAllCampaignFeatures(campaignId: String, forceRefresh: Bool = false) async {
        ...
        mapQualityReport = nil
        mapQualityReportRunId = nil
        ...
        if let cachedBundle = await campaignRepository.getCampaignMapBundle(campaignId: campaignId),
           isActiveCampaignRequest(campaignId: campaignId, requestId: requestId) {
            ...
            self.reconciliationStatus = cachedBundle.metadata?.reconciliation
            self.mapQualityReport = cachedBundle.metadata?.reconciliationReport
            self.mapQualityReportRunId = cachedBundle.metadata?.reconciliation?.runId
            if ["completed", "review_needed"].contains(
                cachedBundle.metadata?.reconciliation?.status.lowercased() ?? ""
            ),
               let runId = self.mapQualityReportRunId,
               let report = self.mapQualityReport {
                MapQualityReportHistoryStore.save(
            ...
            updateBackendOptimizationState(
                ...
                reconciliation: cachedBundle.metadata?.reconciliation,
                requestId: requestId
            )
            ...
            storeCurrentFeatureSnapshot(campaignId: campaignId)
            markCampaignDataLoaded(campaignId: campaignId)
            isLoading = false
            print(
    ...
    private func bundlePreservingLocalBuildingsIfNeeded(
        ...
        let counts = CanonicalCampaignMapBundleCounts(
            ...
            renderVersion: bundle.counts?.renderVersion,
            buildingUnits: bundle.counts?.buildingUnits ?? bundle.buildingUnits?.count
        )
        ...
        return CanonicalCampaignMapBundle(
            ...
            roads: bundle.roads,
            links: bundle.links,
            addressOrphans: bundle.addressOrphans,
            buildingOrphans: bundle.buildingOrphans,
            buildingUnits: bundle.buildingUnits,
            reconciliation: bundle.reconciliation,
            reconciliationReport: bundle.reconciliationReport,
            counts: counts,
    ...
    private func applyCanonicalCampaignMapBundle(
        ...
        self.reconciliationStatus = bundle.reconciliation
        self.mapQualityReport = bundle.reconciliationReport ?? bundle.reconciliation?.report
        self.mapQualityReportRunId = bundle.reconciliation?.runId
        if ["completed", "review_needed"].contains(bundle.reconciliation?.status.lowercased() ?? ""),
           let runId = self.mapQualityReportRunId,
           let report = self.mapQualityReport {
            MapQualityReportHistoryStore.save(
                campaignId: campaignId,
    ...
    private func updateBackendOptimizationState(
        campaignId: String,
        linksStatus: String?,
        reconciliation: CanonicalMapReconciliation?,
        requestId: UUID?
    ) {
        guard isActiveCampaignRequest(campaignId: campaignId, requestId: requestId) else { return }
            ...
            for (attempt, delaySeconds) in retryDelaysSeconds.enumerated() {
                ...
                do {
                    ...
                    await MainActor.run {
                        ...
                        self.reconciliationStatus = status
                        self.isMapDataOptimizing = status.isOptimizing
                    }
                } catch {
                    print(
