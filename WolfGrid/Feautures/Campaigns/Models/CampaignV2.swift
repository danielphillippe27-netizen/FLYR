...
struct CampaignV2: Identifiable, Codable, Equatable {
    ...
    /// The lightweight list receives only the aggregate count, while hydrated
    /// campaign details may also contain address rows. Never under-report when
    /// one source is fresher than the other.
    var houseCount: Int {
        max(totalFlyers, addresses.count)
    }
