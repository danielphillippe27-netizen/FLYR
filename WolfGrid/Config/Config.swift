...
enum Config {
    ...
    static var backendAPIURL: URL {
        if let configured = urlValue(for: "WOLFGRID_API_BASE_URL") {
            return normalizedBackendAPIURL(configured)
        }
        if let configured = urlValue(for: "WOLFGRID_API_URL") {
            return normalizedBackendAPIURL(configured)
        }
    ...
    static var passwordRecoveryLocalCallbackURL: URL {
        urlValue(for: "WOLFGRID_PASSWORD_RECOVERY_DEV_CALLBACK_URL")
            ?? passwordRecoveryWolfGridCallbackURL
    }

    static var passwordRecoveryWolfGridCallbackURL: URL {
        URL(string: "\(isSalesBuild ? "wolfgridsales" : "wolfgrid")://auth/reset-password")!
    }

    static var passwordRecoveryLegacyCallbackURL: URL {
        URL(string: "flyr://auth/reset-password")!
    }

    static var passwordRecoveryRedirectURL: URL {
        if let configured = urlValue(for: "WOLFGRID_PASSWORD_RECOVERY_REDIRECT_URL") {
            return configured
        }
    ...
    static var passwordRecoveryAcceptedURLs: [URL] {
        var urls: [URL] = [
            ...
            passwordRecoveryWolfGridCallbackURL,
            passwordRecoveryLegacyCallbackURL
        ]
...
enum OfflineFirstConfig {
    ...
    static var maxBackgroundPreloadCampaigns: Int {
        max(1, Config.intValue(for: "OFFLINE_FIRST_MAX_BACKGROUND_PRELOAD_CAMPAIGNS", default: 8))
    }
