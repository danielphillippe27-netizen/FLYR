...
OUT=ROOT/'WolfGrid/Resources/WolfyV2'; ART=ROOT/'art/wolfy-v2'
...
mesh_file=OUT/'pup-mesh.bin';mesh_file.write_bytes(struct.pack('<%sf'%len(verts),*verts)+struct.pack('<%sI'%len(tris),*tris))
clip_meta={}
for name,c in clips.items():
 p=OUT/('pup-'+name+'.bin');p.write_bytes(struct.pack('<%sf'%len(c['matrices']),*c['matrices']))
 clip_meta[name]={'file':p.name,'duration':c['duration'],'frames':c['frames']}
manifest={'version':2,'stage':'Wolf Pup','prototype':True,'vertices':len(verts)//16,'indices':len(tris),'bones':names,'mesh':mesh_file.name,'clips':clip_meta}
manifest['files']={p.name:{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in OUT.glob('*.bin')}
(OUT/'pup-manifest.json').write_text(json.dumps(manifest,indent=2))
...
bpy.ops.object.camera_add(location=(2,-3,1.7));cam=bpy.context.object;cam.rotation_euler=(Vector((0,-.1,.5))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='ORTHO';cam.data.ortho_scale=2.4;scene.camera=cam
bpy.ops.wm.save_as_mainfile(filepath=str(ART/'WolfyPupPrototype.blend'))
scene.render.filepath=str(ART/'pup-prototype.png');bpy.ops.render.render(write_still=True)
print('PROTOTYPE',manifest['vertices'],manifest['indices']//3,'triangles')
